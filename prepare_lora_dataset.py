...
def get_key(y, sr):
    # Convert stereo to mono for feature extraction if needed
    if y.ndim > 1:
...
def process_file(file_path, output_dir, dataset_name="BollyHood Beats"):
    ...
    # 2. Dead-Silence Trimming (using librosa, need mono for this to find boundaries, but trim original)
    if y.ndim > 1:
        y_mono = librosa.to_mono(y.T)
    else:
        y_mono = y
        
    # Trim silence (top_db=40 means sounds below 40dB of peak are considered silence)
    ...
    max_samples = 45 * sr
    is_one_shot = duration_s < 3.0  # Under 3 seconds is considered a one-shot
    
    if is_one_shot:
        # One-Shots: Leave exactly as is, do not loop
        pass
    else:
        # Loops: Tile to 45s if less, Trim if more
        if len(y) < max_samples:
            repeats = int(np.ceil(max_samples / len(y)))
            y = np.tile(y, (repeats, 1)) if y.ndim > 1 else np.tile(y, repeats)
            
        # Hard truncate at exactly 45 seconds to prevent memory overflow
        y = y[:max_samples]

    duration_s = len(y) / sr  # Recalculate after shaping
